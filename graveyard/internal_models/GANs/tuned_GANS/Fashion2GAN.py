...
class Fashion2GAN:
    ...
    def __init__(self, returns_df, asset_name, latent_dim=200, window_size=252, quarter_length=200, 
                 batch_size=120, n_epochs=300, lambda_gp=60, lambda_tail=55, lambda_structure=30):
        """
    ...
    def add_new_return(self, new_return, date):
        """
        Add a new return to the returns series and update conditions.
        
        Parameters:
            new_return (float): The new return value to add
            date: The date for the new return (used for indexing if Series)
            
        Returns:
            None: Updates internal state of the model
        """
        ...
        # Update rolling returns and scaler with the new data
        self.rolling_returns, self.scaler = self.create_rolling_returns(self.returns_series)
        ...
        # Ensure conditions align with rolling returns
        min_length = min(len(self.rolling_returns), len(self.conditions))
        self.rolling_returns = self.rolling_returns[-min_length:]
        self.conditions = self.conditions[-min_length:]
        
        print(f"Added new return for {date}. Updated dataset has {len(self.rolling_returns)} windows.")


    def partial_retrain(self, n_epochs=50):
        """
        Perform partial retraining of the model after new data has been added.
        Uses weighted sampling to emphasize recent data.
        
        Parameters:
            n_epochs (int): Number of epochs for partial retraining
            
        Returns:
            dict: Training statistics
        """
        # Setup with weighted sampling (similar to original setup but simplified)
        returns_tensor = torch.tensor(self.rolling_returns, dtype=torch.float32)
        conditions_tensor = torch.tensor(self.conditions, dtype=torch.float32)
        
        # Create time-based weights that emphasize recent data
        time_indices = np.arange(len(self.rolling_returns))
        sample_weights = np.exp(-self.lambda_decay * time_indices[::-1])  # More emphasis on recent data
        sample_weights /= np.sum(sample_weights)
        ...
        # Create dataset and dataloader with the weighted sampler
        dataset = TensorDataset(returns_tensor, conditions_tensor)
        ...
        for epoch in range(n_epochs):
            ...
            for i, (real_returns, cond) in enumerate(dataloader):
                batch_size = real_returns.size(0)
                ...
                epoch_stats['d_losses'].append(d_loss.item())
                ...
                if i % 3 == 0:
                    ...
                    epoch_stats['g_losses'].append(g_loss.item())
                    epoch_stats['tail_penalties'].append(tail_penalty.item())
                    epoch_stats['structure_penalties'].append(structure_penalty.item())

                if i % 10 == 0:
                    print(f"[Partial Retraining] [Epoch {epoch}/{n_epochs}] [Batch {i}/{len(dataloader)}] "
                        f"[D loss: {d_loss.item():.4f}] [G loss: {g_loss.item():.4f}]")
            
            # Collect epoch statistics
            for key in stats:
                if epoch_stats[key]:
                    stats[key].append(np.mean(epoch_stats[key]))
        
        print(f"Partial retraining completed: {n_epochs} epochs")
        return stats


    def adaptive_generate_scenarios(self, save=True, num_scenarios=10000, use_latest_only=False, blend_factor=0.7):
        """
        Generate scenarios with adaptive behavior based on recent market conditions.
        
        Parameters:
            save (bool): Whether to save the generated scenarios
            num_scenarios (int): Number of scenarios to generate
            use_latest_only (bool): If True, only use the latest condition for generation
                                    If False, sample from all conditions with emphasis on recent ones
            blend_factor (float): How much to emphasize recent conditions (0-1)
                                Higher values give more weight to recent conditions
        
        Returns:
            np.ndarray: Generated scenario returns
        """
        self.generator.eval()
        device = 'cuda' if self.cuda else 'cpu'
        ...
        with torch.no_grad():
            if use_latest_only:
                # Use only the latest condition
                rel = self.conditions[-1:]
            else:
                # Use all conditions but with weighted sampling to emphasize recent ones
                rel = self.conditions
                
                # Create time-based weights
                time_indices = np.arange(len(rel))
                # Adjust decay factor to control emphasis on recency
                decay_rate = -np.log(1 - blend_factor) / len(rel)
                condition_weights = np.exp(-decay_rate * time_indices[::-1])
                condition_weights /= np.sum(condition_weights)
            
            for _ in range(num_scenarios // batch_size):
                z = torch.randn(batch_size, self.latent_dim, device=device)
                
                if use_latest_only:
                    # Replicate the latest condition
                    condition_indices = np.zeros(batch_size, dtype=int)
                else:
                    # Sample conditions with emphasis on recent ones
                    condition_indices = np.random.choice(
                        len(rel), 
                        size=batch_size, 
                        replace=True,
                        p=condition_weights
        ...
        if save:
            save_dir = "generated_CGAN_output_test"
            os.makedirs(save_dir, exist_ok=True)
            latest_date = self.returns_series.index[-1] if isinstance(self.returns_series, pd.Series) else "latest"
            save_path = os.path.join(save_dir, f'generated_returns_{self.asset_name}_{latest_date}_scenarios.pt')
            torch.save(torch.tensor(all_generated_returns), save_path)
            print(f"Generated scenarios saved to: {save_path}")
