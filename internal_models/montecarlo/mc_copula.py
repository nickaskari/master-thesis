...
class MonteCarloCopula:
    ...
    def generate_uniform_samples(self, uniform_data):
        t_cop = self.fit_student_copula()
        t_cop.random(self.n_simulations * self.n_days).to_numpy().reshape(self.n_simulations, self.n_days, uniform_data.shape[1])
    ...
    def get_cumulative_returns(self, weights, simulated_returns):
