...
class LambardGAN:
    ...
    def __init__(self, returns_df, asset_name, latent_dim=150, window_size=252, 
                 ...
                 lag_periods=None, cond_scale = 2, lambda_skew=0):
        ...
        self.scaler = LambertW(self.returns_series)
      ...
      for i in range(max_l, n + 1):
          ...
          for lag in self.lag_periods: 
              ...
              condition_vector.extend([
                  float(cum_return), float(volatility), float(kurtosis),
                  float(max_drawdown),
              ])
    ...
    def compute_tail_penalty(self, real_returns, gen_returns):
        ...
        k = int(self.tail_quantile * real_flat.size(0))
        k = max(k, 1)
    ...
    def train(self):
      ...
      for epoch in range(self.n_epochs):
          for i, (real_returns, cond) in enumerate(self.dataloader):
              ...
              self.wass_distance.append(d_loss.item())
              d_loss += gp
    ...
    def generate_scenarios(self, save=True, num_scenarios=50000):
        ...
        with torch.no_grad():
            ...
            for _ in range(num_scenarios // batch_size):
                ...
                #if np.max(gen_returns) > historical_max or np.min(gen_returns) < historical_min:
                 # continue
                
                all_generated_returns.append(gen_returns)
        print(len(all_generated_returns))     
    ...
    def fine_tuning(self, epochs):
        """Fine-tune existing generator and discriminator with updated data"""
        # Reset losses
        self.d_losses = []
        self.g_losses = []
        self.gp_losses = []
        ...
        # Store original scaler params
        original_params = self.scaler.params
        original_returns_max = self.scaler.returns_max
        original_returns_mean = self.scaler.returns_mean
        original_condition_scaler = self.condition_scaler
        
        # Create conditions (will overwrite condition_scaler)
        raw_conditions = self.create_multi_lag_conditions(self.returns_series, self.window_size)
        
        # Restore original condition scaler and transform with it
        self.condition_scaler = original_condition_scaler
        normalized_conditions = self.condition_scaler.transform(raw_conditions) * self.cond_scale
        
        # Transform returns with original scaler params
        returns_np = self.returns_series.values
        returns_norm = returns_np - original_returns_mean
        transformed_returns = self.scaler.W_delta(
            (returns_norm - original_params[0]) / original_params[1], 
            original_params[2]
        ) / original_returns_max
        ...
        for k in range(len(normalized_conditions)):
            idx_condition_ends = max_hist_lag + k - 1
            idx_target_window_starts = idx_condition_ends + 1
            idx_target_window_ends = idx_target_window_starts + self.window_size

            if idx_target_window_ends <= len(transformed_returns):
                target_window = transformed_returns[idx_target_window_starts:idx_target_window_ends]
                aligned_rolling_returns_list.append(target_window)
                aligned_conditions_list.append(normalized_conditions[k])
        ...
        for epoch in range(epochs):
            for i, (real_returns, cond) in enumerate(self.dataloader):
                ...
                # Train Generator every 3 iterations
                if i % 3 == 0:
                    self.optimizer_G.zero_grad()
                    ...
                    skew_penalty = self.compute_skewness_penalty(real_returns, gen_returns_g)
                    g_loss = -torch.mean(self.discriminator(gen_returns_g, cond)) + \
                            self.lambda_tail * tail_penalty + self.lambda_skew * skew_penalty
                    g_loss.backward()
                    self.optimizer_G.step()
                    self.g_losses.append(g_loss.item())
                    self.tail_losses.append(tail_penalty.item())
                else:
                    if self.g_losses:
                        self.g_losses.append(self.g_losses[-1])
                        self.tail_losses.append(self.tail_losses[-1])

        print(f"Fine-tuning complete: {epochs} epochs")
        self.plot_losses()
