...
def calculate_market_scr(
    ...
    liability_value=0.7,
    liability_duration=3
):
    if spread_shocks_corp is None:
        spread_shocks_corp = {'AAA': 0.009, 'AA': 0.011,
                         'A': 0.014, 'BBB': 0.025, 'BB': 0.045, 'B': 0.075}
        
    #if spread_shocks_gov is None:  # Not used with our data
    #    spread_shocks_gov = {'A': 0.011, 'BBB': 0.014, 'BB': 0.025, 'B': 0.045}

    if interest_rate_shocks_up is None:
        interest_rate_shocks_up = {
            1: 0.70, 2: 0.70, 3: 0.64, 4: 0.59, 5: 0.55, 6: 0.52, 7: 0.49, 8: 0.47, 9: 0.44, 10: 0.42, 20: 0.26, 90: 0.20}

    if interest_rate_shocks_down is None:
        interest_rate_shocks_down = {
            1: -0.75, 2: -0.65, 3: -0.56, 4: -0.50, 5: -0.46, 6: -0.42, 7: -0.39, 8: -0.36, 9: -0.33, 10: -0.31, 20: -0.29, 90: -0.20}

    yield_curve = {  # As of summer 2024
        0: 0.0343, 1: 0.0343, 2: 0.0311, 3: 0.0293, 4: 0.0283,
        5: 0.0277, 6: 0.0274, 7: 0.0272, 8: 0.0271, 9: 0.0272, 10: 0.0273
    }

    # Calculate base and shocked PV for assets
    ...
    # Calculate base and shocked PV for liabilities
    ...
    market_scr_equity = np.sqrt(equity_scr_global**2 + equity_scr_other **
                                2 + 2 * 0.75 * equity_scr_global * equity_scr_other)

    market_scr_real_estate = abs(asset_values.get(
        'real_estate', 0) * real_estate_shock)

    market_scr_spread = sum(
