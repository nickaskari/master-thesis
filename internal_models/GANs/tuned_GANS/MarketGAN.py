...
class MarketGAN:
    ...
    def __init__(self, returns_df, asset_name, latent_dim=200, window_size=252, quarter_length=200, 
                 batch_size=120, n_epochs=300, lambda_gp=60, lambda_tail=40, lambda_structure=30):
        """
    ...
    def generate_new_scenarios_from_return(self, new_return, date, save=False, num_scenarios=10000, 
                                        mean_reversion_strength=0.3, anti_cyclical=True):
        """
        Update the GAN with a new return observation and generate scenarios with anti-cyclical properties
        
        Parameters:
            new_return: The new return value
            date: Date of the new return
            save: Whether to save generated scenarios
            num_scenarios: Number of scenarios to generate
            mean_reversion_strength: Strength of mean reversion (0-1)
            anti_cyclical: Whether to apply anti-cyclical adjustments
        
        Returns:
            numpy.ndarray: Generated return scenarios
        """
        # Store original scaler on first call if not already stored
        if not hasattr(self, 'original_scaler'):
            import copy
            self.original_scaler = copy.deepcopy(self.scaler)
        ...
        # Create rolling windows but use original scaler for consistency
        window_size = self.window_size
        scaled_returns = self.original_scaler.transform(self.returns_series.values.reshape(-1, 1))
        
        rolling_returns = []
        for i in range(len(scaled_returns) - window_size + 1):
            window = scaled_returns[i:i + window_size]
            rolling_returns.append(window.flatten())
        self.rolling_returns = np.array(rolling_returns)
        ...
        current_regime = detect_market_regime(self.returns_series)
        
        # Store the current (most recent) condition
        current_condition = self.conditions[-1:][0]
        
        # Apply anti-cyclical adjustments if enabled
        if anti_cyclical:
            # Calculate historical averages of condition metrics
            historical_conditions = self.conditions[:-1]  # All conditions except the current one
            historical_avg_condition = np.mean(historical_conditions, axis=0)
            
            # Calculate specific regime-based adjustments
            if current_regime == "crisis":
                # During crisis, start incorporating recovery expectations
                # We want conditions to lean toward better historical values
                # Calculate mean reversion factor based on the severity of the deviation
                deviations = current_condition - historical_avg_condition
                
                # Focus mainly on cumulative return and drawdown metrics
                # For cumulative return (index 0), move toward historical average
                reversion_factor_return = mean_reversion_strength * deviations[0]
                
                # For drawdown (index 3), move toward historical average
                reversion_factor_drawdown = mean_reversion_strength * deviations[3]
                
                # Find similar historical recovery periods to blend in
                recovery_indices = []
                for i, condition in enumerate(historical_conditions):
                    # Look for conditions with improving returns after drawdowns
                    if i < len(historical_conditions) - 1:
                        next_condition = historical_conditions[i + 1]
                        if condition[3] < -0.1 and next_condition[0] > condition[0]:
                            recovery_indices.append(i)
                
                # If we found recovery periods, blend them into our adjustments
                if recovery_indices:
                    recovery_conditions = historical_conditions[recovery_indices]
                    avg_recovery_condition = np.mean(recovery_conditions, axis=0)
                    # Blend current with recovery-typical conditions
                    blended_condition = current_condition.copy()
                    blended_condition[0] += reversion_factor_return
                    blended_condition[3] += reversion_factor_drawdown
                    
                    # Prepare a generator of blended conditions
                    def generate_conditions(batch_size):
                        # Mix of current, blended, and recovery-like conditions
                        conditions = []
                        for _ in range(batch_size):
                            # Randomize the blend factors
                            if recovery_indices and np.random.random() < 0.4:
                                # 40% chance to use a historical recovery condition
                                historical_idx = np.random.choice(recovery_indices)
                                conditions.append(historical_conditions[historical_idx])
                            elif np.random.random() < 0.6:
                                # 60% chance to use the blended condition
                                conditions.append(blended_condition)
                            else:
                                # Use the current condition
                                conditions.append(current_condition)
                        return np.array(conditions)
                    
            elif current_regime == "recovery":
                # During recovery, start incorporating normal conditions
                # Less aggressive mean reversion
                deviations = current_condition - historical_avg_condition
                reversion_factor = mean_reversion_strength * 0.5 * deviations
                
                blended_condition = current_condition - reversion_factor
                ...
                def generate_conditions(batch_size):
                    ...
                    for _ in range(batch_size):
                        if np.random.random() < 0.7:
                            # 70% chance to use the current condition
                            conditions.append(current_condition)
                        else:
                            # 30% chance to use the blended (more normal) condition
                            conditions.append(blended_condition)
                    return np.array(conditions)
                    
            elif current_regime == "deteriorating":
                # When markets start worsening, start incorporating some crisis preparation
                # Find similar historical periods where conditions deteriorated
                deviations = current_condition - historical_avg_condition
                
                # During deterioration, we want to be more conservative
                # Exaggerate negative trends slightly in some scenarios
                blended_condition = current_condition.copy()
                # Make volatility slightly higher
                blended_condition[1] *= 1.1
                # Make drawdown slightly worse
                blended_condition[3] *= 1.1
                ...
                def generate_conditions(batch_size):
                    ...
                    for _ in range(batch_size):
                        if np.random.random() < 0.6:
                            # 60% chance to use the current condition
                            conditions.append(current_condition)
                        else:
                            # 40% chance to use the more conservative condition
                            conditions.append(blended_condition)
                    return np.array(conditions)
                    
            else:  # "normal" regime
                # During normal times, incorporate mean reversion mildly
                deviations = current_condition - historical_avg_condition
                reversion_factor = mean_reversion_strength * 0.3 * deviations
                ...
                def generate_conditions(batch_size):
                    ...
                    for _ in range(batch_size):
                        if np.random.random() < 0.8:
                            # 80% chance to use the current condition
                            conditions.append(current_condition)
                        else:
                            # 20% chance to use the blended condition
                            conditions.append(blended_condition)
                    return np.array(conditions)
        else:
            # If anti-cyclical is disabled, just use the current condition
            def generate_conditions(batch_size):
                return np.tile(current_condition, (batch_size, 1))
        ...
        with torch.no_grad():
            for _ in range(num_scenarios // batch_size):
                ...
                sampled_conditions = generate_conditions(batch_size)
                cond = torch.tensor(sampled_conditions, dtype=torch.float32, device=device)
                
                gen_returns = self.generator(z, cond)
                
                # Convert back to original scale using the original scaler
                gen_returns_np = gen_returns.cpu().numpy()
                gen_returns_original = self.original_scaler.inverse_transform(gen_returns_np)
        ...
        if save:
            ...
            # Additionally save the regime information
            regime_path = os.path.join(save_dir, f'regime_{self.asset_name}_{date}.txt')
            with open(regime_path, 'w') as f:
                f.write(f"Detected regime: {current_regime}\n")
        
        print(f"Generated scenarios for {date}, detected regime: {current_regime}")
        return all_generated_returns
    
def detect_market_regime(returns_series, window=63, historical_vol=None):
    """
    Detect if we're in a crisis, recovery, or normal market regime
    
    Parameters:
        returns_series: Series of returns
        window: Number of days to look back for regime detection
        historical_vol: Long-term historical volatility benchmark (optional)
    
    Returns:
        str: Market regime ('crisis', 'recovery', or 'normal')
    """
    if len(returns_series) < window:
        return "normal"  # Default if not enough data
    
    recent_returns = returns_series[-window:].values
    
    # Calculate key metrics
    cumulative = np.prod(1 + recent_returns) - 1
    volatility = np.std(recent_returns, ddof=1)
    
    # Calculate drawdown
    cum_returns = np.cumprod(1 + recent_returns)
    peak = np.maximum.accumulate(cum_returns)
    drawdown = (cum_returns / peak - 1).min()
    
    # Calculate long-term volatility if not provided
    if historical_vol is None:
        if len(returns_series) > 252:
            historical_vol = returns_series[-252:-window].std()
        else:
            historical_vol = returns_series[:-window].std() if len(returns_series) > window else volatility
    
    # Positive momentum calculation
    recent_trend = np.mean(returns_series[-20:].values)
    
    # Simple logic for regime detection
    if drawdown < -0.15 and volatility > 1.5 * historical_vol:
        return "crisis"
    elif drawdown < -0.07 and cumulative > 0.05 and recent_trend > 0:
        return "recovery"
    elif volatility > 1.3 * historical_vol and recent_trend < 0:
        return "deteriorating"
    else:
        return "normal"

def find_similar_historical_periods(current_window, historical_windows, n_matches=5):
    """
    Find historical periods that are similar to the current one
    
    Parameters:
        current_window: Current window of returns or conditions
        historical_windows: Array of historical windows
        n_matches: Number of similar periods to return
    
    Returns:
        list: Indices of similar historical periods
    """
    if len(historical_windows) <= n_matches:
        return list(range(len(historical_windows)))
    
    # Calculate Euclidean distance between current window and all historical windows
    distances = []
    for i, window in enumerate(historical_windows):
        # Normalize both windows before comparison
        norm_current = (current_window - np.mean(current_window)) / (np.std(current_window) + 1e-8)
        norm_window = (window - np.mean(window)) / (np.std(window) + 1e-8)
        
        # Calculate distance (use sum of squared differences)
        distance = np.sum((norm_current - norm_window) ** 2)
        distances.append((i, distance))
    
    # Sort by distance and return indices of the most similar windows
    similar_periods = sorted(distances, key=lambda x: x[1])[:n_matches]
    return [idx for idx, _ in similar_periods]
