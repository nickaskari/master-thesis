...
def garch_noise(batch_size, latent_dim):
    # Fit a small GARCH(1,1) model on synthetic noise
    model = arch_model(np.random.randn(1000), vol='Garch', p=1, q=1)
    fitted_model = model.fit(disp="off")

    # Reduce the horizon to a manageable size (e.g., 100 instead of batch_size * latent_dim)
    horizon = min(100, latent_dim)  # Limit forecast size to avoid memory explosion

    # Generate volatility forecasts for a smaller horizon
    forecast = fitted_model.forecast(start=0, horizon=horizon)

    # Extract variance, take square root to get standard deviation
    simulated_vol = np.sqrt(forecast.variance.values[-1, :])

    # Generate standard normal noise and scale it by the GARCH volatility
    garch_noise = simulated_vol * np.random.randn(batch_size, horizon)

    # Expand to latent_dim by repeating noise patterns if needed
    garch_noise = np.tile(garch_noise, (1, latent_dim // horizon + 1))[:, :latent_dim]

    # Convert to PyTorch tensor
    return torch.tensor(garch_noise).float()
    ...
    def train(self):
        ...
        for epoch in range(self.opt.n_epochs):
            for i, (real_returns,) in enumerate(self.dataloader):
                ...
                z = garch_noise(batch_size, self.opt.latent_dim)
